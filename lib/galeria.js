...
function lerEstado(caminho) {
  try {
    ...
    // Migração silenciosa: galerias criadas antes das categorias existirem
    // não têm esse campo -- garante que sempre existe, pra não quebrar
    // nada que espere um array aqui.
    estado.galerias.forEach((g) => { if (!Array.isArray(g.categorias)) g.categorias = []; });
    return estado;
  } catch (e) {
    return { galerias: [] };
  }
...
function adicionarFoto(caminho, projetoId, { nomeExibicao, tag, tipo, r2Key, capturaIdOrigem, categoriaId }) {
  ...
  const foto = {
    ...
    nomeExibicao: (nomeExibicao && nomeExibicao.trim()) || 'Sem nome',
...
function editarFoto(caminho, projetoId, fotoId, { nomeExibicao, tag, ordem, arquivada, categoriaId }) {
  ...
  if (nomeExibicao !== undefined) foto.nomeExibicao = nomeExibicao.trim() || foto.nomeExibicao;
...
module.exports = {
  ...
  adicionarFoto, editarFoto, excluirFoto,
